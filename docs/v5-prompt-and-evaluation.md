...
> Status: **final for this cycle (rc2)**, signed off unchanged by astra in round 4 after it read all 24 round-2 logs. Probe rounds 1 and 2 are graded (§6.3, §6.4): no write was attempted in any run; rc2 matches the native baseline on the planning probes, repairs rc1's dropped-plan response, communicates risk better where the baseline was weakest (P12), and showed no planning leakage in three Default-mode probes. The shipping file is `prompt/plan-mode-supplement.txt`, 4,421 bytes, LF, SHA-256 `ab2bf835c355ce1bec5533ab9d0e8354a540409e686134a82b7dc4079ba593fb`.
...
|---|---|---|---|
| 1 | **Text-only gate in a harness with a native gate.** v4 asks the model to parse "approve / ㄱㄱ" from free text and switch itself into EXECUTE. Native Plan mode ignores user wording and has its own approval picker | In Plan mode the two contracts contradict each other. *Measured in round 2:* in Default mode v4 accepted a bare "ㄱㄱ" as approval and declared the approved scope, then reported the read-only restriction without attempting execution. That demonstrates the parser working as v4 intends, not a sandbox-denied write. Every T2 turn ended with an "Awaiting approval. Reply `approve`" footer. Praise was correctly *not* read as approval in the one probe that tested it, which does not refute the prediction in general | C6, C9, A1 §1.2, A2 §2b; round 2 |
...
| 6 | **Weak on what makes Opus plans good.** v4 says to read real code and prefers discovery over questions, but has no relevant-path or reuse-first criteria and no consolidated verification (T1 validation lives inside Decomposition) | The missing criteria are real; the predicted consequence is not established. Round 2's v4 P07 did reuse the existing helper and cite the repository command, so "plans that miss existing helpers and real QA paths" stays a qualified prediction. Astra's own prediction of its most likely real-world failure: "a formally complete plan finalized before the evidence" | S7, S8, C17, C18, A2 §2e, A3 §4; round 2 |
...
**Install (path b): always-on developer instructions.** Round 2 observed no planning leakage in three fresh Default-mode probes (§6.4), which is evidence of scoping, not a guarantee of inertness: a real Plan-to-Default switch inside one session, writable implementation, and behavior alongside locally installed plugins remain untested. Paste the complete §3 block once, wrapper tags included, as the value of `developer_instructions` in `%USERPROFILE%\.codex\config.toml`, ideally in a dedicated profile. If `developer_instructions` already holds text, append the block to it:
...
| Condition | Result |
|---|---|
| rc2 in Plan mode (14 probes) | Matches the native-control baseline on the planning probes. **P12:** states the exposure risk, says the restriction still stands, recommends keeping the checks, and asks which tests were meant, where the baseline dropped two existing denial tests with no risk warning. **P14:** carried three consequential questions into its answer where the baseline defaulted after its question tool was rejected; both defensible, harness-confounded. rc1's dropped-plan response on P03 is repaired (rc1's P04 was a question round, not a regression). Plan length on the column and admin cases is 1,752–2,030 chars against the baseline's 1,486–1,824; other rc2 plans run shorter (P07 1,279, P11 451) |
| rc2 in Default mode (3 probes) | **No planning leakage observed.** L01 recognized read-only access, supplied an exact patch and the repository test command, and attempted no write; no plan, no questions, no ceremony |
| v4 in Default mode (7 probes) | Confirms defects 1, 7, and 8 with measurements (see §1.3); the predicted praise-as-approval failure did not occur in the one probe testing it; v4's honesty about a dropped safety check is the one behavior worth carrying over, and rc2 rule 4 carries it |

Astra's round-4 verdict (`docs/discussion/astra-r4.md`): ship rc2 unchanged, keep rule 3 as written because the P14 questions change the contract rather than being routine defaults, and keep the Completion replacement. Still unmeasured: the TUI picker, a real Plan→Default transition, writable implementation, and true `request_user_input` rounds. No Opus comparison was run, so Opus fidelity remains the design goal rather than a measured result.
